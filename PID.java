...
public class PID {
    ...
    private double REFRESH_RATE = 50; // robot cycle time
	 ...
	 * @param feedforward whether or not to cummulate calculations.
	 * @param name the name of the pid object, used for NetworkTables posting.
	 */
	public PID(double p, double i, double d, double feedforward, boolean inverted, String name, boolean debug) {
		this.name = name;
		this.p = p;
		this.i = i;
		this.d = d;
		...
		this.deltaTime = (long)(1.0/REFRESH_RATE*1000);
	...
	/**
	 * Returns the feed forward value of the PID controller.
	 * 
	...
	/**
	 * Returns the error of the most recent Calc().
	 * 
	 * @return double the current target value.
	 */
	...
	public double calc(double input) {
		...
		//Calculate output
		double output = p*error + i*sumError + d*derivative + feedforward;
		int sign = -1;
		if(output > 0) 
			sign = 1;
		output = Math.abs(output)+ min;
		output *= sign;
		output = Math.min(Math.max(output, Outmin), Outmax);   //Contrain the output between min and max range
